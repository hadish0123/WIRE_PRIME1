                ...
                for inbound in inbounds:
                    ...
                    try:
                        ...
                        for peer in peers:
                            ...
                            status="pending" if current[2]==0 else "active"
                            print(f"VPN_PEER node={node.id} inbound={inbound.id} peer={peer.get('public_key')} status={status} handshake={current[2]} endpoint={peer.get('endpoint')} rx={current[0]} tx={current[1]} delta_rx={delta_in} delta_tx={delta_out}", flush=True)
