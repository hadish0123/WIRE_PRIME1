...
def render_wireguard(c:WireGuardConfig)->str:
 ...
 for p in c.peers:
  _key(p.public_key);ipaddress.ip_network(p.allowed_ips,strict=False);lines += ["","[Peer]",f"PublicKey = {_key(p.public_key)}",f"AllowedIPs = {p.allowed_ips}"]
 ...
 return "\n".join(lines)+"\n"
def render_openvpn(server_network:str,port:int,transport:str="udp",tls_min:str="1.2",tls_crypt_path:str="/etc/primevpn/tls-crypt.key",ca_path:str="/etc/primevpn/ca.crt",cert_path:str="/etc/primevpn/server.crt",key_path:str="/etc/primevpn/server.key",crl_path:str|None=None)->str:
 net=ipaddress.ip_network(server_network,strict=False)
