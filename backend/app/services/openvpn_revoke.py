...
def revoke_certificate(existing_crl_pem,client_cert_pem,ca_key_pem,ca_cert_pem):
 ...
 builder=x509.CertificateRevocationListBuilder().issuer_name(ca.subject).last_update(datetime.now(timezone.utc)).next_update(datetime.now(timezone.utc)+timedelta(days=30))
 if existing_crl_pem:
