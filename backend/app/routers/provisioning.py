...
def bootstrap(node_id:str,admin:Admin=Depends(current_admin),db:Session=Depends(get_db)):
 ...
 task=ProvisioningTask(tenant_id=admin.tenant_id,node_id=n.id,idempotency_key="bootstrap:"+raw,state=NodeState.authenticating.value,bootstrap_token_hash=h,bootstrap_expires_at=datetime.now(timezone.utc)+__import__("datetime").timedelta(minutes=15))
...
def exchange(task_id:str,token:str,db:Session=Depends(get_db)):
 ...
 task.bootstrap_token_hash=None;task.bootstrap_expires_at=None;task.state=NodeState.syncing.value
